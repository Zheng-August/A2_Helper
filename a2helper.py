...
def find_bridges_in_radius(bridges: list[list], lat: float, lon: float,
    ...
    for bridge in bridges:
        bridge_id = bridge[COLUMN_ID]
        bridge_lat = bridge[COLUMN_LAT]
        bridge_lon = bridge[COLUMN_LON]

        if bridge_id not in exclusions and \
        calculate_distance(lat, lon, bridge_lat, bridge_lon) <= radius:
            in_circle.append(bridge_id)
...
def get_bridge_condition(bridges: list[list], bridge_id: int) -> float:
    ...
    for i in range(len(bridge[COLUMN_BCI][INDEX_BCI_SCORES])):
        if bridge[COLUMN_BCI][INDEX_BCI_SCORES][i] > 0:
...
def inspect_bridge(bridges: list[list], bridge_id: int, inspect_date: str,
    ...
    for bridge in bridges:
        if bridge[COLUMN_ID] == bridge_id:
            ...
            year = inspect_date[6:]
            bridge[COLUMN_BCI][INDEX_BCI_YEARS].insert(0, year)
...
def find_worst_bci(bridges: list[list], bridge_ids: list[int]) -> int:
    ...
    worst_bci = get_bridge_condition(bridges, bridge_ids[0])

    for bridge_id in bridge_ids[1:]:
        bci = get_bridge_condition(bridges, bridge_id)
...
def map_route(bridges: list[list], lat: float, lon: float,
    ...
    while i < max_bridges:
        available_bridges = \
            find_bridges_in_radius(bridges, lat, lon, radius, visited)

        if available_bridges != []:
            worst_bridge_id = find_worst_bci(bridges, available_bridges)
            bridge = find_bridge_by_id(bridges, worst_bridge_id)
            lat, lon = bridge[COLUMN_LAT], bridge[COLUMN_LON]
            visited.append(worst_bridge_id)
        i += 1
    return visited
...
def clean_span_data(raw_spans: str) -> list[float]:
    ...
    check_button = False
    equal_num = 0
    for char in raw_spans:
        if check_button: 
            if char == ';':
                lst.append(clean_length_data(num_str))
                num_str = ''
                check_button = False
            else:
                num_str += char
